...
func writeOpenAISSEChunk(c *gin.Context, chunk []byte) {
...
func (h *OpenAIAPIHandler) ChatCompletions(c *gin.Context) {
	...
	rawJSON = maybeSanitizeFreshClaudeCLISession(c, rawJSON)
...
func maybeSanitizeFreshClaudeCLISession(c *gin.Context, rawJSON []byte) []byte {
	if c == nil || c.Request == nil || len(rawJSON) == 0 {
		return rawJSON
	}
	if !wantsFreshClaudeCLISession(c.Request) {
		return rawJSON
	}
	sanitized, changed := sanitizeFreshClaudeCLIChatCompletions(rawJSON)
	if !changed {
		return rawJSON
	}
	return sanitized
...
func wantsFreshClaudeCLISession(req *http.Request) bool {
	if req == nil {
		return false
	}
	if !isTruthyHeaderValue(req.Header.Get("X-Covs-Fresh-Session")) {
		return false
	}

	userAgent := strings.ToLower(strings.TrimSpace(req.Header.Get("User-Agent")))
	xApp := strings.ToLower(strings.TrimSpace(req.Header.Get("X-App")))
	if !strings.Contains(userAgent, "claude-cli/") {
		return false
	}
	return strings.Contains(userAgent, "external, cli") || xApp == "cli"
}

func isTruthyHeaderValue(raw string) bool {
...
func sanitizeFreshClaudeCLIChatCompletions(rawJSON []byte) ([]byte, bool) {
	...
	lastResumeIndex := -1
	for i := range items {
		if messageContainsClaudeCLIResumeBootstrap(items[i]) {
			lastResumeIndex = i
		}
	}
	if lastResumeIndex < 0 {
		return rawJSON, false
	}

	out := "[]"
	for i := range items {
		if strings.EqualFold(strings.TrimSpace(items[i].Get("role").String()), "system") {
			out, _ = sjson.SetRaw(out, "-1", items[i].Raw)
		}
	}

	lastUser := ""
	for i := lastResumeIndex; i < len(items); i++ {
		if !strings.EqualFold(strings.TrimSpace(items[i].Get("role").String()), "user") {
			continue
		}
		if sanitized, ok := sanitizeFreshClaudeCLIUserMessage(items[i]); ok {
			lastUser = sanitized
		}
	}
	if lastUser != "" {
		out, _ = sjson.SetRaw(out, "-1", lastUser)
	}

	updated, err := sjson.SetRawBytes(rawJSON, "messages", []byte(out))
	if err != nil {
		return rawJSON, false
	}
	return updated, true
...
func messageContainsClaudeCLIResumeBootstrap(message gjson.Result) bool {
	content := message.Get("content")
	switch {
	case content.Type == gjson.String:
		return looksLikeClaudeCLIResumeBootstrapText(content.String())
	case content.IsArray():
		found := false
		content.ForEach(func(_, part gjson.Result) bool {
			if looksLikeClaudeCLIResumeBootstrapText(part.Get("text").String()) {
				found = true
				return false
			}
			return true
...
func sanitizeFreshClaudeCLIUserMessage(message gjson.Result) (string, bool) {
	content := message.Get("content")
	switch {
	case content.Type == gjson.String:
		cleaned := strings.TrimSpace(stripClaudeCLIResumeBootstrapText(content.String()))
		if cleaned == "" {
			return "", false
		}
		updated, err := sjson.Set(message.Raw, "content", cleaned)
		if err != nil {
			return "", false
		}
		return updated, true
	case content.IsArray():
		...
		content.ForEach(func(_, part gjson.Result) bool {
			text := part.Get("text")
			if !text.Exists() {
				out, _ = sjson.SetRaw(out, "-1", part.Raw)
				kept++
				return true
			}

			cleaned := strings.TrimSpace(stripClaudeCLIResumeBootstrapText(text.String()))
			if cleaned == "" {
				return true
			}

			updatedPart, err := sjson.Set(part.Raw, "text", cleaned)
			if err != nil {
				return true
			}
			out, _ = sjson.SetRaw(out, "-1", updatedPart)
			kept++
			return true
		})
		if kept == 0 {
		...
		updated, err := sjson.SetRaw(message.Raw, "content", out)
		if err != nil {
			return "", false
		}
		return updated, true
...
func stripClaudeCLIResumeBootstrapText(text string) string {
	trimmed := strings.TrimSpace(text)
	if trimmed == "" {
		return ""
	}
	if looksLikeClaudeCLIResumeBootstrapText(trimmed) {
		return ""
	}
	return trimmed
}

func looksLikeClaudeCLIResumeBootstrapText(text string) bool {
...
func (h *OpenAIAPIHandler) handleCompletionsStreamingResponse(c *gin.Context, rawJSON []byte) {
	...
	for {
		...
		case chunk, ok := <-dataChan:
			if !ok {
				setSSEHeaders()
				writeOpenAISSEChunk(c, []byte("[DONE]"))
				flusher.Flush()
				cliCancel(nil)
				return
			}
			...
			if converted != nil {
				writeOpenAISSEChunk(c, converted)
				flusher.Flush()
			}
