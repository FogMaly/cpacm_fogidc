...
func (e *compactCaptureExecutor) ExecuteStream(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (<-chan coreexecutor.StreamChunk, error) {
	...
	ch <- coreexecutor.StreamChunk{Payload: []byte("data: {\"ok\":true}\n\n")}
	close(ch)
	return ch, nil
}
...
func TestOpenAIResponsesFallsBackToAcceptEventStream(t *testing.T) {
	gin.SetMode(gin.TestMode)
	executor := &compactCaptureExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)
	...
	if executor.calls != 0 {
		t.Fatalf("non-stream calls = %d, want 0", executor.calls)
	}
	if executor.streamCalls != 1 {
		t.Fatalf("stream calls = %d, want 1", executor.streamCalls)
	}
	if !strings.Contains(resp.Body.String(), "data: {\"ok\":true}") {
		t.Fatalf("body = %q, want SSE payload", resp.Body.String())
	}
}

func TestOpenAIResponsesRejectsAcceptSSEWithExplicitStreamFalse(t *testing.T) {
	gin.SetMode(gin.TestMode)
	executor := &compactCaptureExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)
	...
	if !strings.Contains(resp.Body.String(), "Accept requests SSE but body sets stream=false") {
		t.Fatalf("body = %q, want mismatch error", resp.Body.String())
	}
}
