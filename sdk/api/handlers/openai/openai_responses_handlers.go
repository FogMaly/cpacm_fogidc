...
func (h *OpenAIResponsesAPIHandler) Responses(c *gin.Context) {
	...
	stream, errResp := resolveResponsesStreamMode(c, rawJSON)
	if errResp != nil {
...
func resolveResponsesStreamMode(c *gin.Context, rawJSON []byte) (bool, *handlers.ErrorResponse) {
	streamResult := gjson.GetBytes(rawJSON, "stream")
	acceptsSSE := acceptsEventStream(c.GetHeader("Accept"))

	if streamResult.Exists() {
		stream := streamResult.Bool()
		if !stream && acceptsSSE {
			return false, &handlers.ErrorResponse{
				Error: handlers.ErrorDetail{
					Message: "Request is inconsistent: Accept requests SSE but body sets stream=false",
					Type:    "invalid_request_error",
				},
			}
		}
