...
func TestGetRequestDetails_PreservesSuffix(t *testing.T) {
	...
	modelRegistry.RegisterClient("test-request-details-codex", "codex", []*registry.ModelInfo{
		{ID: "gpt-5.3-codex", Created: now + 15},
	})
	modelRegistry.RegisterClient("test-request-details-nowcoding", "codex", []*registry.ModelInfo{
		{ID: "nowcoding/gpt-5.4", Created: now + 14},
	})
	modelRegistry.RegisterClient("test-request-details-claude", "claude", []*registry.ModelInfo{
		{ID: "claude-sonnet-4-5", Created: now + 5},
	})
	modelRegistry.RegisterClient("test-request-details-prefixed-claude", "claude", []*registry.ModelInfo{
		{ID: "covs/claude-sonnet-4-6", Created: now + 4},
	})
	...
	clientIDs := []string{
		...
		"test-request-details-nowcoding",
		"test-request-details-claude",
		"test-request-details-prefixed-claude",
	}
	...
	tests := []struct {
		...
		{
			name:          "prefixed upstream codex model resolves provider",
			inputModel:    "openai/gpt-5.3-codex(high)",
			wantProviders: []string{"codex"},
			wantModel:     "openai/gpt-5.3-codex(high)",
			wantErr:       false,
		},
		{
			name:          "prefixed registered codex model resolves provider",
			inputModel:    "nowcoding/gpt-5.4(high)",
			wantProviders: []string{"codex"},
			wantModel:     "nowcoding/gpt-5.4(high)",
			wantErr:       false,
		},
		{
		...
		{
			name:          "prefixed claude model resolves bare external model",
		...
		{
			name:          "prefixed claude channel model preserves exact route",
			inputModel:    "yunyi-claude/claude-sonnet-4-6(high)",
			wantProviders: []string{"claude"},
			wantModel:     "yunyi-claude/claude-sonnet-4-6(high)",
			wantErr:       false,
		},
	}
