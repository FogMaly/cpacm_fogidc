...
func requestExecutionMetadata(ctx context.Context) map[string]any {
	...
	key := ""
	strictPublicRouting := false
	sessionAffinitySeed := ""
	if ctx != nil {
		if ginCtx, ok := ctx.Value("gin").(*gin.Context); ok && ginCtx != nil && ginCtx.Request != nil {
			key = strings.TrimSpace(ginCtx.GetHeader("Idempotency-Key"))
			strictPublicRouting = ginCtx.GetBool("cpamc_public_strict")
			sessionAffinitySeed = syntheticSessionAffinitySeed(ginCtx.Request)
		}
	}
	if key == "" {
		key = uuid.NewString()
	...
	if strictPublicRouting {
		meta[coreexecutor.StrictPublicModelRoutingMetadataKey] = true
	}
	if sessionAffinitySeed != "" {
		meta[coreexecutor.SessionAffinitySeedMetadataKey] = sessionAffinitySeed
	}
	return meta
}
...
func syntheticSessionAffinitySeed(r *http.Request) string {
	if r == nil {
		return ""
	}
	parts := make([]string, 0, 3)
	if tokenFingerprint := requestIdentityFingerprint(r); tokenFingerprint != "" {
	...
	if userAgent := strings.TrimSpace(r.UserAgent()); userAgent != "" {
		parts = append(parts, "ua:"+digestAffinitySeed(userAgent))
	}
	if len(parts) == 0 {
		return ""
	}
	return "fallback:" + digestAffinitySeed(strings.Join(parts, "|"))
}

func requestIdentityFingerprint(r *http.Request) string {
	if r == nil {
		return ""
	}
	for _, header := range []string{"Authorization", "X-API-Key", "Api-Key"} {
		if value := strings.TrimSpace(r.Header.Get(header)); value != "" {
			return digestAffinitySeed(value)
		}
	}
	return ""
}

func digestAffinitySeed(raw string) string {
...
func requestedModelForExecution(ctx context.Context, normalizedModel string) string {
	normalizedModel = strings.TrimSpace(normalizedModel)
	if ctx != nil {
		if ginCtx, ok := ctx.Value("gin").(*gin.Context); ok && ginCtx != nil {
			if requested := strings.TrimSpace(ginCtx.GetString("cpams_requested_model")); requested != "" {
				return requested
			}
		}
	}
...
func (h *BaseAPIHandler) GetContextWithCancel(handler interfaces.APIHandler, c *gin.Context, ctx context.Context) (context.Context, APIHandlerCancelFunc) {
	...
	return newCtx, func(params ...interface{}) {
		if h.Cfg.RequestLog && len(params) == 1 {
			if existingBytes := runtimeexecutor.MaterializeAPIResponse(c); len(bytes.TrimSpace(existingBytes)) > 0 {
			...
			if len(payload) > 0 {
				if existingBytes := runtimeexecutor.MaterializeAPIResponse(c); len(existingBytes) > 0 {
				...
				runtimeexecutor.AppendAPIResponseLog(c, payload, runtimeexecutor.RequestLogMaxBodyBytes(h.Cfg))
			}
		}
...
func (h *BaseAPIHandler) ExecuteWithAuthManager(ctx context.Context, handlerType, modelName string, rawJSON []byte, alt string) ([]byte, *interfaces.ErrorMessage) {
	...
	reqMeta[coreexecutor.RequestedModelMetadataKey] = requestedModelForExecution(ctx, normalizedModel)
	payload := rawJSON
	if len(payload) == 0 {
		payload = nil
	}
...
func (h *BaseAPIHandler) getRequestDetails(modelName string) (providers []string, normalizedModel string, err *interfaces.ErrorMessage) {
	...
	// CPAMC and other routing layers may resolve a stable public model to a
	// provider-prefixed upstream model (for example "openai/gpt-5.3-codex").
	// Keep the resolved model unchanged for downstream execution, but fall back
	// to provider lookup using the upstream portion so the request still routes
	// through the correct handler/provider pool.
	if len(providers) == 0 {
		for _, candidate := range []string{baseModel, resolvedModelName} {
			candidate = strings.TrimSpace(candidate)
			if candidate == "" {
				continue
			}
			if idx := strings.Index(candidate, "/"); idx >= 0 && idx < len(candidate)-1 {
				upstream := strings.TrimSpace(candidate[idx+1:])
				if upstream == "" {
					continue
				}
				providers = util.GetProviderName(upstream)
				if len(providers) > 0 {
					break
				}
			}
...
func (h *BaseAPIHandler) WriteErrorResponse(c *gin.Context, msg *interfaces.ErrorMessage) {
	...
	previous := runtimeexecutor.MaterializeAPIResponse(c)
	runtimeexecutor.AppendAPIResponseLog(c, body, runtimeexecutor.RequestLogMaxBodyBytes(h.Cfg))
