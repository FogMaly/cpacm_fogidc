...
func TestHealthAwareSelectorPick_PrefersHealthierAuth(t *testing.T) {
	t.Parallel()

	selector := &HealthAwareSelector{}
	auths := []*Auth{
		{ID: "a", Runtime: AuthHealthMetrics{SuccessCount: 1, FailureCount: 5, Quota429Count: 3, AvgLatencyMS: 900}},
		{ID: "b", Runtime: AuthHealthMetrics{SuccessCount: 9, FailureCount: 1, Quota429Count: 0, AvgLatencyMS: 120}},
	}

	got, err := selector.Pick(context.Background(), "claude", "m1", cliproxyexecutor.Options{}, auths)
	if err != nil {
		t.Fatalf("Pick() error = %v", err)
	}
	if got == nil || got.ID != "b" {
		t.Fatalf("Pick() auth = %#v, want auth b", got)
	}
}

func TestHealthAwareSelectorPick_PrefersUnifiedHealthScore(t *testing.T) {
	t.Parallel()
	...
	auths := []*Auth{
		{
			ID: "a",
			Runtime: AuthSelectionRuntime{
				Metrics: AuthHealthMetrics{SuccessCount: 9, FailureCount: 1, AvgLatencyMS: 120},
				Unified: health.UnifiedScore{Score: 42, Status: "degraded", SignalsPresent: 3},
			},
		},
		{
			ID: "b",
			Runtime: AuthSelectionRuntime{
				Metrics: AuthHealthMetrics{SuccessCount: 6, FailureCount: 1, AvgLatencyMS: 180},
				Unified: health.UnifiedScore{Score: 84, Status: "healthy", SignalsPresent: 3},
			},
		},
	}

	got, err := selector.Pick(context.Background(), "codex", "gpt-5.4-pro", cliproxyexecutor.Options{}, auths)
	if err != nil {
		t.Fatalf("Pick() error = %v", err)
	}
	if got == nil || got.ID != "b" {
...
func TestHealthAwareSelectorPick_PrefersRecentSelectionWithinSmallScoreDelta(t *testing.T) {
	t.Parallel()
	...
	auths := []*Auth{
		{
			...
			Runtime: AuthSelectionRuntime{
				Metrics: AuthHealthMetrics{
					...
					FailureCount:   1,
					AvgLatencyMS:   140,
					LastSelectedAt: now.Add(-20 * time.Second),
				},
				Unified: health.UnifiedScore{Score: 78, Status: "healthy", SignalsPresent: 3},
			},
		},
		{
			ID: "b",
			Runtime: AuthSelectionRuntime{
				Metrics: AuthHealthMetrics{
					...
					FailureCount:   1,
					AvgLatencyMS:   120,
					LastSelectedAt: now.Add(-10 * time.Minute),
				},
				Unified: health.UnifiedScore{Score: 82, Status: "healthy", SignalsPresent: 3},
			},
		},
	}

	got, err := selector.Pick(context.Background(), "claude", "claude-opus-4-6", cliproxyexecutor.Options{}, auths)
	if err != nil {
		t.Fatalf("Pick() error = %v", err)
	}
	if got == nil || got.ID != "a" {
		t.Fatalf("Pick() auth = %#v, want sticky recent auth a", got)
	}
}

func TestHealthAwareSelectorPick_SwitchesWhenScoreGapIsLarge(t *testing.T) {
	t.Parallel()
	...
	auths := []*Auth{
		{
			...
			Runtime: AuthSelectionRuntime{
				...
				Unified: health.UnifiedScore{Score: 60, Status: "degraded", SignalsPresent: 3},
			},
		},
		{
			ID: "b",
			Runtime: AuthSelectionRuntime{
				...
				Unified: health.UnifiedScore{Score: 88, Status: "healthy", SignalsPresent: 3},
			},
		},
	}
	...
	if got == nil || got.ID != "b" {
		t.Fatalf("Pick() auth = %#v, want healthier auth b", got)
	}
}
...
func TestIsAuthBlockedForModel_ExcludedWildcardIsBlocked(t *testing.T) {
	t.Parallel()

	blocked, reason, next := isAuthBlockedForModel(&Auth{
		ID: "a",
		Attributes: map[string]string{
			"excluded_models": "*",
		},
	}, "test-model(high)", time.Now())
	if !blocked {
