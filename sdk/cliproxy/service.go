...
func (s *Service) Run(ctx context.Context) error {
	...
	startConfirm := time.NewTimer(250 * time.Millisecond)
	defer startConfirm.Stop()
	select {
	...
	reloadCallback := func(newCfg *config.Config) {
		...
		if s.coreManager != nil && previousStrategy != nextStrategy {
			...
			case "health-aware":
				selector = &coreauth.HealthAwareSelector{}
			default:
...
func (s *Service) registerModelsForAuth(a *coreauth.Auth) {
	...
	case "gemini":
		...
		if entry := s.resolveConfigGeminiKey(a); entry != nil {
			...
			if authKind == "apikey" {
				excluded = normalizeConfiguredModelExclusions(entry.Models, entry.ExcludedModels, s.cfg != nil && s.cfg.ForceModelPrefix)
			}
		}
...
func normalizeConfiguredModelExclusions[T modelEntry](configured []T, excluded []string, forceModelPrefix bool) []string {
	if len(configured) == 0 || len(excluded) == 0 || !forceModelPrefix {
		return excluded
	}
...
func buildConfigModels[T modelEntry](models []T, ownedBy, modelType string) []*ModelInfo {
	...
	out := make([]*ModelInfo, 0, len(models)*2)
	seen := make(map[string]struct{}, len(models)*2)
	addModel := func(id, display, thinkingSource string) {
		...
		if source := strings.TrimSpace(thinkingSource); source != "" {
			if upstream := registry.LookupStaticModelInfo(source); upstream != nil && upstream.Thinking != nil {
				info.Thinking = upstream.Thinking
			}
		}
		if info.Thinking == nil {
			if upstream := registry.LookupStaticModelInfo(id); upstream != nil && upstream.Thinking != nil {
				info.Thinking = upstream.Thinking
			}
		}
		out = append(out, info)
	...
	for i := range models {
		...
		primaryDisplay := name
		if primaryDisplay == "" {
			primaryDisplay = alias
		}
		if alias != "" {
			addModel(alias, primaryDisplay, name)
		}
		if name != "" && !strings.EqualFold(name, alias) {
			addModel(name, name, name)
		}
		if alias == "" && name != "" {
			addModel(name, name, name)
		}
