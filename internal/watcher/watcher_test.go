...
func TestAddOrUpdateClientSkipsNonAuthJSON(t *testing.T) {
	tmpDir := t.TempDir()
	healthFile := filepath.Join(tmpDir, "model-health.json")
	if err := os.WriteFile(healthFile, []byte(`{"updated_at":"2026-04-08T22:31:45Z","models":[]}`), 0o644); err != nil {
		t.Fatalf("failed to create non-auth json file: %v", err)
	}
	...
	w.addOrUpdateClient(healthFile)

	if got := atomic.LoadInt32(&reloads); got != 0 {
		t.Fatalf("expected no reload callback for non-auth json, got %d", got)
	}
	if got := len(w.lastAuthHashes); got != 0 {
		t.Fatalf("expected no hash cache entries for non-auth json, got %d", got)
	}
}
...
func TestHandleEventIgnoresModelHealthSnapshots(t *testing.T) {
	tmpDir := t.TempDir()
	authDir := filepath.Join(tmpDir, "auth")
	if err := os.MkdirAll(authDir, 0o755); err != nil {
		t.Fatalf("failed to create auth dir: %v", err)
	...
	healthFile := filepath.Join(authDir, "model-health.json")
	if err := os.WriteFile(healthFile, []byte(`{"updated_at":"2026-04-08T22:31:45Z","models":[]}`), 0o644); err != nil {
	...
	w.handleEvent(fsnotify.Event{Name: healthFile, Op: fsnotify.Write})
	if atomic.LoadInt32(&reloads) != 0 {
		t.Fatalf("expected no reloads for model-health snapshot, got %d", reloads)
	}
	if got := len(w.lastAuthHashes); got != 0 {
		t.Fatalf("expected snapshot file to stay out of auth hash cache, got %d", got)
	}
}
...
func TestLoadFileClientsSkipsNonAuthJSON(t *testing.T) {
	tmpDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(tmpDir, "auth.json"), []byte(`{"type":"qwen"}`), 0o644); err != nil {
		t.Fatalf("failed to write auth file: %v", err)
	}
	if err := os.WriteFile(filepath.Join(tmpDir, "model-health.json"), []byte(`{"updated_at":"2026-04-08T22:31:45Z","models":[]}`), 0o644); err != nil {
	...
	count := w.loadFileClients(cfg)
	if count != 1 {
		t.Fatalf("expected only auth json files to be counted, got %d", count)
	}
}
