...
func configAuthLabel(configuredName, fallback string) string {
...
func (s *ConfigSynthesizer) synthesizeGeminiKeys(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.GeminiKey {
		...
		a := &coreauth.Auth{
			ID:         id,
			Provider:   "gemini",
			Label:      configAuthLabel(entry.Name, "gemini-apikey"),
			Prefix:     prefix,
...
func (s *ConfigSynthesizer) synthesizeClaudeKeys(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.ClaudeKey {
		...
		if ck.NativePassthrough {
			attrs["native_passthrough"] = "true"
		}
		...
		a := &coreauth.Auth{
			ID:         id,
			Provider:   "claude",
			Label:      configAuthLabel(ck.Name, "claude-apikey"),
			Prefix:     prefix,
			Status:     coreauth.StatusActive,
			ProxyURL:   proxyURL,
			Attributes: attrs,
...
func (s *ConfigSynthesizer) synthesizeCodexKeys(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.CodexKey {
		...
		if protocols := strings.TrimSpace(ck.SupportedProtocols); protocols != "" {
			attrs["supported_protocols"] = protocols
		}
		if transport := strings.TrimSpace(ck.ResponsesNonStreamTransport); transport != "" {
			attrs["responses_nonstream_transport"] = transport
		}
		...
		a := &coreauth.Auth{
			ID:         id,
			Provider:   "codex",
			Label:      configAuthLabel(ck.Name, "codex-apikey"),
			Prefix:     prefix,
			Status:     coreauth.StatusActive,
			ProxyURL:   proxyURL,
			Attributes: attrs,
...
func (s *ConfigSynthesizer) synthesizeVertexCompat(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.VertexCompatAPIKey {
		...
		a := &coreauth.Auth{
			ID:         id,
			Provider:   providerName,
			Label:      configAuthLabel(compat.Name, "vertex-apikey"),
			Prefix:     prefix,
			Status:     coreauth.StatusActive,
			ProxyURL:   proxyURL,
			Attributes: attrs,
