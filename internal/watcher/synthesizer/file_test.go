...
func TestFileSynthesizer_Synthesize_APIKeyMetadataMappedToAttributes(t *testing.T) {
	tempDir := t.TempDir()
	...
	data, _ := json.Marshal(authData)
	err := os.WriteFile(filepath.Join(tempDir, "qwen-apikey.json"), data, 0644)
	if err != nil {
		t.Fatalf("failed to write auth file: %v", err)
	}
	...
	if got := auths[0].Attributes["api_key"]; got != "sk-sp-test" {
		t.Fatalf("expected api_key to be mapped, got %q", got)
	}
	if got := auths[0].Attributes["base_url"]; got != "https://coding.dashscope.aliyuncs.com/v1" {
		t.Fatalf("expected base_url to be mapped, got %q", got)
	}
	if got := auths[0].Provider; got != "qwen" {
		t.Fatalf("expected provider qwen, got %q", got)
	}
	if kind, value := auths[0].AccountInfo(); kind != "api_key" || value != "sk-sp-test" {
		t.Fatalf("expected api_key account info, got kind=%q value=%q", kind, value)
	}
}
