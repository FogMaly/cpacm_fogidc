...
func TestConfigSynthesizer_ClaudeKeys_NativePassthrough(t *testing.T) {
	synth := NewConfigSynthesizer()
	ctx := &SynthesisContext{
		Config: &config.Config{
			ClaudeKey: []config.ClaudeKey{
				{
					APIKey:            "sk-ant-api-raw",
					BaseURL:           "https://api.anthropic.com",
	...
	if got := auths[0].Attributes["native_passthrough"]; got != "true" {
		t.Fatalf("native_passthrough = %q, want true", got)
	}
}
...
func TestConfigSynthesizer_UsesConfiguredChannelNamesAsLabels(t *testing.T) {
	synth := NewConfigSynthesizer()
	ctx := &SynthesisContext{
		Config: &config.Config{
			GeminiKey: []config.GeminiKey{
			...
			ClaudeKey: []config.ClaudeKey{
				{APIKey: "claude-key", Name: "Claude Main"},
			},
			CodexKey: []config.CodexKey{
				{APIKey: "codex-key", Name: "Codex Main"},
			},
			VertexCompatAPIKey: []config.VertexCompatKey{
				{APIKey: "vertex-key", BaseURL: "https://vertex.api", Name: "Vertex Main"},
			},
		},
		Now:         time.Now(),
		IDGenerator: NewStableIDGenerator(),
	...
	for _, auth := range auths {
		...
		if auth.Label != expected {
			t.Fatalf("provider %q label = %q, want %q", auth.Provider, auth.Label, expected)
		}
	}
}

func TestConfigSynthesizer_CodexSupportedProtocolsPropagatesToAuthAttributes(t *testing.T) {
	synth := NewConfigSynthesizer()
	ctx := &SynthesisContext{
		Config: &config.Config{
			CodexKey: []config.CodexKey{
	...
	if got := auths[0].Attributes["supported_protocols"]; got != "responses, chat/completions" {
		t.Fatalf("supported_protocols = %q, want %q", got, "responses, chat/completions")
	}
}

func TestConfigSynthesizer_CodexResponsesNonStreamTransportPropagatesToAuthAttributes(t *testing.T) {
	synth := NewConfigSynthesizer()
	ctx := &SynthesisContext{
		Config: &config.Config{
			CodexKey: []config.CodexKey{
				{
					APIKey:                      "codex-key",
					BaseURL:                     "https://example.com/v1",
					ResponsesNonStreamTransport: "sse",
				},
			},
		},
		Now:         time.Now(),
	...
	if got := auths[0].Attributes["responses_nonstream_transport"]; got != "sse" {
		t.Fatalf("responses_nonstream_transport = %q, want %q", got, "sse")
	}
}
