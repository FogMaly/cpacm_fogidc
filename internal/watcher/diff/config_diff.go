...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.Routing.Engine != newCfg.Routing.Engine {
		changes = append(changes, fmt.Sprintf("routing.engine: %s -> %s", oldCfg.Routing.Engine, newCfg.Routing.Engine))
	}
	if oldCfg.Routing.Strategy != newCfg.Routing.Strategy {
		changes = append(changes, fmt.Sprintf("routing.strategy: %s -> %s", oldCfg.Routing.Strategy, newCfg.Routing.Strategy))
	}
	if !reflect.DeepEqual(normalizeIntMap(oldCfg.Routing.ProviderConcurrency), normalizeIntMap(newCfg.Routing.ProviderConcurrency)) {
		changes = append(changes, "routing.provider-concurrency: updated")
	}
	if oldCfg.InboundRateLimit.PerKeyQPS != newCfg.InboundRateLimit.PerKeyQPS {
		changes = append(changes, fmt.Sprintf("inbound-rate-limit.per-key-qps: %v -> %v", oldCfg.InboundRateLimit.PerKeyQPS, newCfg.InboundRateLimit.PerKeyQPS))
	}
	if oldCfg.InboundRateLimit.GlobalConcurrency != newCfg.InboundRateLimit.GlobalConcurrency {
		changes = append(changes, fmt.Sprintf("inbound-rate-limit.global-concurrency: %d -> %d", oldCfg.InboundRateLimit.GlobalConcurrency, newCfg.InboundRateLimit.GlobalConcurrency))
	}
...
func normalizeIntMap(in map[string]int) map[string]int {
	if len(in) == 0 {
		return nil
	}
