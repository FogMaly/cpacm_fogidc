...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		Routing:                config.RoutingConfig{Engine: "v1"},
		InboundRateLimit:       config.InboundRateLimitConfig{PerKeyQPS: 0, GlobalConcurrency: 0},
	...
	newCfg := &config.Config{
		...
		Routing:                config.RoutingConfig{Engine: "v2", ProviderConcurrency: map[string]int{"claude": 8}},
		InboundRateLimit:       config.InboundRateLimitConfig{PerKeyQPS: 5, GlobalConcurrency: 100},
		ClaudeKey: []config.ClaudeKey{
	...
	expectContains(t, details, "routing.engine: v1 -> v2")
	expectContains(t, details, "routing.provider-concurrency: updated")
	expectContains(t, details, "inbound-rate-limit.per-key-qps: 0 -> 5")
	expectContains(t, details, "inbound-rate-limit.global-concurrency: 0 -> 100")
