...
type ipv4PreferredDialer struct {
	dialer *net.Dialer
}

func (d *ipv4PreferredDialer) Dial(network, addr string) (net.Conn, error) {
	...
	if strings.HasPrefix(network, "tcp") {
		if conn, err := d.dialer.Dial("tcp4", addr); err == nil {
			return conn, nil
		}
	}
...
func newUtlsRoundTripper(cfg *config.SDKConfig) *utlsRoundTripper {
	baseDialer := &ipv4PreferredDialer{
		dialer: &net.Dialer{
	...
	if cfg != nil && cfg.ProxyURL != "" {
		...
		if err != nil {
			...
			pDialer, err := proxy.FromURL(proxyURL, baseDialer)
			if err != nil {
