...
const (
	...
	nativePanelMarker            = `data-cpapi-native-ai-providers=`
...
func EnsureLatestManagementHTML(ctx context.Context, staticDir string, proxyURL string, panelRepository string) bool {
	...
	_, _, _ = sfGroup.Do(localPath, func() (interface{}, error) {
		...
		if !localFileMissing && hasPinnedNativeManagementAsset(localPath) {
			log.Debug("management asset sync skipped: local native control panel asset is pinned")
			return nil, nil
		}
...
func hasPinnedNativeManagementAsset(path string) bool {
	raw, err := os.ReadFile(path)
	if err != nil {
		return false
	}
	return bytes.Contains(bytes.ToLower(raw), []byte(nativePanelMarker))
}
