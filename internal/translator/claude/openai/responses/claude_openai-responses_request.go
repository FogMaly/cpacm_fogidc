...
func ConvertOpenAIResponsesRequestToClaude(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	userID := resolveClaudeResponsesMetadataUserID(root)
	...
	if input := root.Get("input"); input.Exists() && input.Type == gjson.String {
		...
		input.ForEach(func(_, item gjson.Result) bool {
					...
					parts.ForEach(func(_, part gjson.Result) bool {
						...
						case "input_text", "output_text":
							if t := part.Get("text"); t.Exists() {
								txt := strings.TrimSpace(t.String())
								if txt != "" {
			...
			case "custom_tool_call":
				...
				toolUse, _ = sjson.Set(toolUse, "id", callID)
				toolUse, _ = sjson.Set(toolUse, "name", name)
				toolUse, _ = sjson.SetRaw(toolUse, "input", claudeResponsesWrapCustomToolInput(item.Get("input").String()))
			...
			case "function_call_output":
				...
				callID := item.Get("call_id").String()
				outputStr := claudeResponsesToolOutputToString(item.Get("output"))
	...
	if tools := root.Get("tools"); tools.Exists() && tools.IsArray() {
		...
		tools.ForEach(func(_, tool gjson.Result) bool {
			...
			if params := tool.Get("parameters"); params.Exists() {
				tJSON, _ = sjson.SetRaw(tJSON, "input_schema", util.SanitizeToolSchemaJSON(params.Raw))
			} else if params = tool.Get("parametersJsonSchema"); params.Exists() {
				tJSON, _ = sjson.SetRaw(tJSON, "input_schema", util.SanitizeToolSchemaJSON(params.Raw))
			} else if strings.EqualFold(tool.Get("type").String(), "custom") {
				tJSON, _ = sjson.SetRaw(tJSON, "input_schema", claudeResponsesCustomToolSchema())
			}
...
func resolveClaudeResponsesMetadataUserID(root gjson.Result) string {
