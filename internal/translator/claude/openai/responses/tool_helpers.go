...
func claudeResponsesToolKindByName(originalRequestRawJSON []byte, name string) string {
...
func claudeResponsesCustomToolSchema() string {
	return `{"type":"object","properties":{"input":{"type":"string"}},"required":["input"]}`
}

func claudeResponsesWrapCustomToolInput(input string) string {
	inputJSON := gjson.AppendJSONString(nil, input)
	return `{"input":` + string(inputJSON) + `}`
}

func claudeResponsesUnwrapCustomToolInput(raw string) string {
...
func claudeResponsesToolOutputToString(output gjson.Result) string {
	if !output.Exists() {
		return ""
	}
	switch {
...
func claudeResponsesToolItemID(kind, callID string) string {
	if kind == "custom" {
		return "ctc_" + callID
	}
	return "fc_" + callID
