...
func ConvertClaudeResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) []string {
	...
	case "content_block_start":
		...
		if typ == "text" {
			...
			toolKind := claudeResponsesToolKindByName(originalRequestRawJSON, name)
			...
			item, _ = sjson.Set(item, "item.id", claudeResponsesToolItemID(toolKind, st.CurrentFCID))
	...
	case "content_block_delta":
		...
		if dt == "text_delta" {
			...
			if pj := d.Get("partial_json"); pj.Exists() {
				...
				if claudeResponsesToolKindByName(originalRequestRawJSON, st.FuncNames[idx]) != "custom" {
					msg := `{"type":"response.function_call_arguments.delta","sequence_number":0,"item_id":"","output_index":0,"delta":""}`
					msg, _ = sjson.Set(msg, "sequence_number", nextSeq())
					msg, _ = sjson.Set(msg, "item_id", claudeResponsesToolItemID("function", st.CurrentFCID))
	...
	case "content_block_stop":
		...
		if st.InTextBlock {
			...
			toolKind := claudeResponsesToolKindByName(originalRequestRawJSON, st.FuncNames[idx])
			if toolKind == "custom" {
				fcDone := `{"type":"response.custom_tool_call_input.done","sequence_number":0,"item_id":"","output_index":0,"input":""}`
				fcDone, _ = sjson.Set(fcDone, "sequence_number", nextSeq())
				fcDone, _ = sjson.Set(fcDone, "item_id", claudeResponsesToolItemID(toolKind, st.CurrentFCID))
				fcDone, _ = sjson.Set(fcDone, "output_index", idx)
				fcDone, _ = sjson.Set(fcDone, "input", claudeResponsesUnwrapCustomToolInput(args))
				out = append(out, emitEvent("response.custom_tool_call_input.done", fcDone))
			} else {
			...
			itemDone, _ = sjson.Set(itemDone, "item.id", claudeResponsesToolItemID(toolKind, st.CurrentFCID))
			itemDone, _ = sjson.Set(itemDone, "item.call_id", st.CurrentFCID)
			itemDone, _ = sjson.Set(itemDone, "item.name", st.FuncNames[idx])
			if toolKind == "custom" {
				itemDone, _ = sjson.Set(itemDone, "item.input", claudeResponsesUnwrapCustomToolInput(args))
			} else {
		...
		if len(st.FuncArgsBuf) > 0 {
			...
			for _, idx := range idxs {
				...
				if toolKind == "custom" {
					...
					item, _ = sjson.Set(item, "input", claudeResponsesUnwrapCustomToolInput(args))
				} else {
					item, _ = sjson.Set(item, "arguments", args)
				}
				item, _ = sjson.Set(item, "id", claudeResponsesToolItemID(toolKind, callID))
...
func ConvertClaudeResponseToOpenAIResponsesNonStream(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) string {
	...
	if len(toolCalls) > 0 {
		...
		for _, i := range idxs {
			...
			toolKind := claudeResponsesToolKindByName(originalRequestRawJSON, st.name)
			item := `{"id":"","type":"function_call","status":"completed","arguments":"","call_id":"","name":""}`
			if toolKind == "custom" {
				item = `{"id":"","type":"custom_tool_call","status":"completed","call_id":"","name":"","input":""}`
				item, _ = sjson.Set(item, "input", claudeResponsesUnwrapCustomToolInput(args))
			...
			item, _ = sjson.Set(item, "id", claudeResponsesToolItemID(toolKind, st.id))
