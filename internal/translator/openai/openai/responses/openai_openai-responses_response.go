...
type oaiToResponsesState struct {
	...
	FuncArgsBuf     map[int]*strings.Builder // logical tool index -> args
	FuncNames       map[int]string           // logical tool index -> name
	FuncCallIDs     map[int]string           // logical tool index -> call_id
	FuncOutputIndex map[int]int              // logical tool index -> output_index
...
func responsesToolItemID(kind, callID string) string {
	prefix := "fc_"
	if kind == "custom" {
		prefix = "ctc_"
	}
...
func responsesToolOutputItemAdded(kind string) string {
	if kind == "custom" {
		return `{"type":"response.output_item.added","sequence_number":0,"output_index":0,"item":{"id":"","type":"custom_tool_call","status":"in_progress","call_id":"","name":"","input":""}}`
	}
	return `{"type":"response.output_item.added","sequence_number":0,"output_index":0,"item":{"id":"","type":"function_call","status":"in_progress","arguments":"","call_id":"","name":""}}`
}

func responsesToolOutputItemDone(kind string) string {
	if kind == "custom" {
		return `{"type":"response.output_item.done","sequence_number":0,"output_index":0,"item":{"id":"","type":"custom_tool_call","status":"completed","call_id":"","name":"","input":""}}`
	}
	return `{"type":"response.output_item.done","sequence_number":0,"output_index":0,"item":{"id":"","type":"function_call","status":"completed","arguments":"","call_id":"","name":""}}`
}

func responsesToolInputDelta(kind string) string {
	if kind == "custom" {
		return `{"type":"response.custom_tool_call_input.delta","sequence_number":0,"item_id":"","output_index":0,"delta":""}`
	}
	return `{"type":"response.function_call_arguments.delta","sequence_number":0,"item_id":"","output_index":0,"delta":""}`
}

func responsesToolInputDone(kind string) string {
	if kind == "custom" {
		return `{"type":"response.custom_tool_call_input.done","sequence_number":0,"item_id":"","output_index":0,"input":""}`
	}
	return `{"type":"response.function_call_arguments.done","sequence_number":0,"item_id":"","output_index":0,"arguments":""}`
}
...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) []string {
	...
	messageOutputIndex := func(choiceIndex int) int {
		if idx, ok := st.MsgOutputIndex[choiceIndex]; ok {
			return idx
		}
		idx := nextOutputIndex()
		st.MsgOutputIndex[choiceIndex] = idx
		return idx
	}
	functionOutputIndex := func(logicalToolIndex int) int {
		if idx, ok := st.FuncOutputIndex[logicalToolIndex]; ok {
			return idx
		}
		idx := nextOutputIndex()
		st.FuncOutputIndex[logicalToolIndex] = idx
		return idx
	}
	var out []string
		...
		choices.ForEach(func(_, choice gjson.Result) bool {
			...
			if delta.Exists() {
				if c := delta.Get("content"); c.Exists() && c.String() != "" {
					...
					msgOutputIndex := messageOutputIndex(idx)
					if !st.MsgItemAdded[idx] {
						item := `{"type":"response.output_item.added","sequence_number":0,"output_index":0,"item":{"id":"","type":"message","status":"in_progress","content":[],"role":"assistant"}}`
						item, _ = sjson.Set(item, "sequence_number", nextSeq())
						item, _ = sjson.Set(item, "output_index", msgOutputIndex)
					...
					if !st.MsgContentAdded[idx] {
						...
						part, _ = sjson.Set(part, "output_index", msgOutputIndex)
					...
					msg, _ = sjson.Set(msg, "output_index", msgOutputIndex)
				...
				if tcs := delta.Get("tool_calls"); tcs.Exists() && tcs.IsArray() {
					...
					if st.MsgItemAdded[idx] && !st.MsgItemDone[idx] {
						...
						done, _ = sjson.Set(done, "output_index", msgOutputIndex)
						...
						partDone, _ = sjson.Set(partDone, "output_index", msgOutputIndex)
						...
						itemDone, _ = sjson.Set(itemDone, "output_index", msgOutputIndex)
					...
					tcs.ForEach(func(_, tc gjson.Result) bool {
						...
						if existingCallID == "" && newCallID != "" {
							...
							functionOutputIndex(funcIdx)
							shouldEmitItem = true
						}

						if shouldEmitItem && effectiveCallID != "" {
							toolKind := responsesToolKindByName(originalRequestRawJSON, st.FuncNames[funcIdx])
							o := responsesToolOutputItemAdded(toolKind)
							o, _ = sjson.Set(o, "sequence_number", nextSeq())
							o, _ = sjson.Set(o, "output_index", functionOutputIndex(funcIdx))
							o, _ = sjson.Set(o, "item.id", responsesToolItemID(toolKind, effectiveCallID))
						...
						if args := tc.Get("function.arguments"); args.Exists() && args.String() != "" {
							...
							if refCallID != "" {
								toolKind := responsesToolKindByName(originalRequestRawJSON, st.FuncNames[funcIdx])
								if toolKind == "custom" {
									st.FuncArgsBuf[funcIdx].WriteString(args.String())
									return true
								}
								ad := responsesToolInputDelta(toolKind)
								ad, _ = sjson.Set(ad, "sequence_number", nextSeq())
								ad, _ = sjson.Set(ad, "item_id", responsesToolItemID(toolKind, refCallID))
								ad, _ = sjson.Set(ad, "output_index", functionOutputIndex(funcIdx))
			...
			if fr := choice.Get("finish_reason"); fr.Exists() && fr.String() != "" {
				...
				if len(st.MsgItemAdded) > 0 {
					...
					for _, i := range idxs {
						if st.MsgItemAdded[i] && !st.MsgItemDone[i] {
							...
							msgOutputIndex := messageOutputIndex(i)
							done := `{"type":"response.output_text.done","sequence_number":0,"item_id":"","output_index":0,"content_index":0,"text":"","logprobs":[]}`
							done, _ = sjson.Set(done, "sequence_number", nextSeq())
				...
				if len(st.FuncCallIDs) > 0 {
					...
					for _, i := range idxs {
						...
						toolKind := responsesToolKindByName(originalRequestRawJSON, st.FuncNames[i])
						fcDone := responsesToolInputDone(toolKind)
						fcDone, _ = sjson.Set(fcDone, "sequence_number", nextSeq())
						fcDone, _ = sjson.Set(fcDone, "item_id", responsesToolItemID(toolKind, callID))
						fcDone, _ = sjson.Set(fcDone, "output_index", functionOutputIndex(i))
						if toolKind == "custom" {
							fcDone, _ = sjson.Set(fcDone, "input", responsesUnwrapCustomToolInput(args))
							out = append(out, emitRespEvent("response.custom_tool_call_input.done", fcDone))
						} else {
							fcDone, _ = sjson.Set(fcDone, "arguments", args)
							out = append(out, emitRespEvent("response.function_call_arguments.done", fcDone))
						}

						itemDone := responsesToolOutputItemDone(toolKind)
						itemDone, _ = sjson.Set(itemDone, "sequence_number", nextSeq())
						itemDone, _ = sjson.Set(itemDone, "output_index", functionOutputIndex(i))
						itemDone, _ = sjson.Set(itemDone, "item.id", responsesToolItemID(toolKind, callID))
						itemDone, _ = sjson.Set(itemDone, "item.call_id", callID)
						itemDone, _ = sjson.Set(itemDone, "item.name", st.FuncNames[i])
						if toolKind == "custom" {
							itemDone, _ = sjson.Set(itemDone, "item.input", responsesUnwrapCustomToolInput(args))
						} else {
				...
				outputsWrapper := `{"arr":[]}`
				itemsByOutputIndex := map[int]string{}
				for _, r := range st.Reasonings {
					...
					itemsByOutputIndex[r.OutputIndex] = item
				}
				for logicalIdx, outputIdx := range st.MsgOutputIndex {
					txt := ""
					if b := st.MsgTextBuf[logicalIdx]; b != nil {
						txt = b.String()
					}
					item := `{"id":"","type":"message","status":"completed","content":[{"type":"output_text","annotations":[],"logprobs":[],"text":""}],"role":"assistant"}`
					item, _ = sjson.Set(item, "id", fmt.Sprintf("msg_%s_%d", st.ResponseID, logicalIdx))
					item, _ = sjson.Set(item, "content.0.text", txt)
					itemsByOutputIndex[outputIdx] = item
				}
				for logicalIdx, outputIdx := range st.FuncOutputIndex {
					args := ""
					if b := st.FuncArgsBuf[logicalIdx]; b != nil {
						args = b.String()
					}
					callID := st.FuncCallIDs[logicalIdx]
					name := st.FuncNames[logicalIdx]
					toolKind := responsesToolKindByName(originalRequestRawJSON, name)
					itemType := "function_call"
					item := `{"id":"","type":"function_call","status":"completed","arguments":"","call_id":"","name":""}`
					if toolKind == "custom" {
						itemType = "custom_tool_call"
						item = `{"id":"","type":"custom_tool_call","status":"completed","call_id":"","name":"","input":""}`
						item, _ = sjson.Set(item, "input", responsesUnwrapCustomToolInput(args))
					} else {
						item, _ = sjson.Set(item, "arguments", args)
					}
					item, _ = sjson.Set(item, "type", itemType)
					item, _ = sjson.Set(item, "id", responsesToolItemID(toolKind, callID))
					item, _ = sjson.Set(item, "call_id", callID)
					item, _ = sjson.Set(item, "name", name)
					itemsByOutputIndex[outputIdx] = item
				}
				if len(itemsByOutputIndex) > 0 {
					idxs := make([]int, 0, len(itemsByOutputIndex))
					for idx := range itemsByOutputIndex {
					...
					for _, idx := range idxs {
						outputsWrapper, _ = sjson.SetRaw(outputsWrapper, "arr.-1", itemsByOutputIndex[idx])
					}
				}
