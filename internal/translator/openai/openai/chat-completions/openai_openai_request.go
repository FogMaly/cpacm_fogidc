...
func ConvertOpenAIRequestToOpenAI(modelName string, inputRawJSON []byte, _ bool) []byte {
	...
	root := gjson.ParseBytes(updatedJSON)
	...
	if tools.Exists() && tools.IsArray() {
		tools.ForEach(func(key, tool gjson.Result) bool {
			...
			if parameters := tool.Get("function.parameters"); parameters.Exists() {
				path := fmt.Sprintf("tools.%d.function.parameters", index)
				if updatedJSON, err = sjson.SetRawBytes(updatedJSON, path, []byte(util.SanitizeToolSchemaJSON(parameters.Raw))); err != nil {
					return true
				}
			}

			if parameters := tool.Get("parameters"); parameters.Exists() {
				path := fmt.Sprintf("tools.%d.parameters", index)
				if updatedJSON, err = sjson.SetRawBytes(updatedJSON, path, []byte(util.SanitizeToolSchemaJSON(parameters.Raw))); err != nil {
					return true
				}
			}
