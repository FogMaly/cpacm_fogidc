...
func ConvertOpenAIResponseToClaude(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) []string {
	...
	if !streamResult.Exists() || (streamResult.Exists() && streamResult.Type == gjson.False) {
		return convertOpenAINonStreamingToAnthropic(rawJSON, requestedModel)
	} else {
		return convertOpenAIStreamingChunkToAnthropic(rawJSON, requestedModel, (*param).(*ConvertOpenAIResponseToAnthropicParams))
	}
}

// convertOpenAIStreamingChunkToAnthropic converts OpenAI streaming chunk to Anthropic streaming events
func convertOpenAIStreamingChunkToAnthropic(rawJSON []byte, requestedModel string, param *ConvertOpenAIResponseToAnthropicParams) []string {
	...
	if param.Model == "" {
		param.Model = normalizedRequestedModel(requestedModel, root.Get("model").String())
	}
	...
	if delta := root.Get("choices.0.delta"); delta.Exists() {
		...
		if reasoning := delta.Get("reasoning_content"); reasoning.Exists() && requestedModelAllowsThinking(requestedModel) {
...
func convertOpenAINonStreamingToAnthropic(rawJSON []byte, requestedModel string) []string {
	root := gjson.ParseBytes(rawJSON)

	out := `{"id":"","type":"message","role":"assistant","model":"","content":[],"stop_reason":null,"stop_sequence":null,"usage":{"input_tokens":0,"output_tokens":0}}`
	out, _ = sjson.Set(out, "id", root.Get("id").String())
	out, _ = sjson.Set(out, "model", normalizedRequestedModel(requestedModel, root.Get("model").String()))
	...
	out = appendResponsesAPIContentToAnthropic(out, root, requestedModel)
...
func ConvertOpenAIResponseToClaudeNonStream(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) string {
	...
	out := `{"id":"","type":"message","role":"assistant","model":"","content":[],"stop_reason":null,"stop_sequence":null,"usage":{"input_tokens":0,"output_tokens":0}}`
	out, _ = sjson.Set(out, "id", root.Get("id").String())
	out, _ = sjson.Set(out, "model", normalizedRequestedModel(requestModelForCompatibility(modelName, originalRequestRawJSON), root.Get("model").String()))
	...
	if choices := root.Get("choices"); choices.Exists() && choices.IsArray() && len(choices.Array()) > 0 {
		...
		if message := choice.Get("message"); message.Exists() {
			if contentResult := message.Get("content"); contentResult.Exists() {
				if contentResult.IsArray() {
					...
					for _, item := range contentResult.Array() {
						...
						case "reasoning":
							if requestedModelAllowsThinking(modelName) {
								flushText()
			...
			if reasoning := message.Get("reasoning_content"); reasoning.Exists() && requestedModelAllowsThinking(modelName) {
				for _, reasoningText := range collectOpenAIReasoningTexts(reasoning) {
					if reasoningText == "" {
						continue
					}
	...
	out = appendResponsesAPIContentToAnthropic(out, root, modelName)
...
func appendResponsesAPIContentToAnthropic(out string, root gjson.Result, requestedModel string) string {
	appendOutputItems := func(items gjson.Result, current string) string {
		...
		for _, item := range items.Array() {
			...
			for _, part := range content.Array() {
				...
				case "text", "output_text":
					...
					current, _ = sjson.SetRaw(current, "content.-1", block)
	...
	out = appendOutputItems(root.Get("output"), out)
	out = appendOutputItems(root.Get("response.output"), out)
	return out
}

func normalizedRequestedModel(requestedModel, fallback string) string {
