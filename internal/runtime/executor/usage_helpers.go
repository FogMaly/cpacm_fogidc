...
func parseClaudeUsage(data []byte) usage.Detail {
	...
	detail := usage.Detail{
		InputTokens:  usageNode.Get("input_tokens").Int(),
		OutputTokens: usageNode.Get("output_tokens").Int(),
		// Claude reports cache read/write tokens separately. For internal usage
		// accounting we aggregate them so local "total tokens" reflects the
		// full input processed for the request.
