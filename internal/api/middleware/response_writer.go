...
const streamingResponseLogBufferSize = 16
...
type ResponseWriterWrapper struct {
	...
	bodyLimit           int                        // bodyLimit caps how many response bytes are retained for request logs.
	bodyTruncated       bool                       // bodyTruncated records whether the response log body hit the configured cap.
...
func NewResponseWriterWrapper(w gin.ResponseWriter, logger logging.RequestLogger, requestInfo *RequestInfo, bodyLimit int) *ResponseWriterWrapper {
...
func (w *ResponseWriterWrapper) Write(data []byte) (int, error) {
	...
	if w.shouldBufferResponseBody() {
		runtimeexecutor.AppendTruncatedLogBytes(w.body, data, w.bodyLimit, &w.bodyTruncated, "client response body")
	}
...
func (w *ResponseWriterWrapper) WriteString(data string) (int, error) {
	...
	if w.shouldBufferResponseBody() {
		runtimeexecutor.AppendTruncatedLogBytes(w.body, []byte(data), w.bodyLimit, &w.bodyTruncated, "client response body")
	}
	return n, err
}
...
func (w *ResponseWriterWrapper) WriteHeader(statusCode int) {
	...
	if w.isStreaming && w.logger.IsEnabled() {
		...
		if err == nil {
			...
			w.chunkChannel = make(chan []byte, streamingResponseLogBufferSize)
...
func (w *ResponseWriterWrapper) extractAPIResponse(c *gin.Context) []byte {
	return runtimeexecutor.MaterializeAPIResponse(c)
}
