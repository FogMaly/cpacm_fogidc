...
func preserveUnnamedEntryNames[T any](
	incoming []T,
	existing []T,
	...
	existingNamesByIdentity := make(map[string]string, len(existing))
	for i := range existing {
		namePtr := getName(&existing[i])
		name := strings.TrimSpace(*namePtr)
		if name == "" {
			continue
		}
		if key := strings.TrimSpace(identity(existing[i])); key != "" {
			existingNamesByIdentity[key] = name
		}
	}

	sameLength := len(incoming) == len(existing)
	for i := range incoming {
		namePtr := getName(&incoming[i])
		if strings.TrimSpace(*namePtr) != "" {
			continue
		}

		if i < len(existing) {
			oldKey := strings.TrimSpace(identity(existing[i]))
			newKey := strings.TrimSpace(identity(incoming[i]))
			if oldKey != "" && oldKey == newKey {
				*namePtr = strings.TrimSpace(*getName(&existing[i]))
				if strings.TrimSpace(*namePtr) != "" {
					continue
				}
			}
		...
		if key := strings.TrimSpace(identity(incoming[i])); key != "" {
			if preserved, ok := existingNamesByIdentity[key]; ok && strings.TrimSpace(preserved) != "" {
				*namePtr = preserved
				continue
			}
		}

		if sameLength && i < len(existing) {
			*namePtr = strings.TrimSpace(*getName(&existing[i]))
		}
	}
}

func geminiKeyIdentity(entry config.GeminiKey) string {
	return strings.TrimSpace(entry.APIKey) + "|" + strings.TrimSpace(entry.BaseURL)
}

func claudeKeyIdentity(entry config.ClaudeKey) string {
	return strings.TrimSpace(entry.APIKey) + "|" + strings.TrimSpace(entry.BaseURL)
}

func codexKeyIdentity(entry config.CodexKey) string {
	return strings.TrimSpace(entry.APIKey) + "|" + strings.TrimSpace(entry.BaseURL)
}

func vertexCompatKeyIdentity(entry config.VertexCompatKey) string {
	return strings.TrimSpace(entry.APIKey) + "|" + strings.TrimSpace(entry.BaseURL)
}
...
func (h *Handler) PutGeminiKeys(c *gin.Context) {
	...
	preserveUnnamedEntryNames(arr, h.cfg.GeminiKey, func(entry *config.GeminiKey) *string {
		return &entry.Name
	}, geminiKeyIdentity)
...
func (h *Handler) PatchGeminiKey(c *gin.Context) {
	type geminiKeyPatch struct {
		...
		UpstreamModelUpdateAutoSyncEnabled *bool              `json:"upstream_model_update_auto_sync_enabled"`
	}
	var body struct {
		Index *int            `json:"index"`
	...
	if body.Value.Groups != nil {
		entry.Groups = normalizeStringSlice(*body.Value.Groups)
	}
	if body.Value.Tag != nil {
		entry.Tag = strings.TrimSpace(*body.Value.Tag)
	}
	if body.Value.Remark != nil {
		entry.Remark = strings.TrimSpace(*body.Value.Remark)
	}
	if body.Value.Weight != nil {
		entry.Weight = *body.Value.Weight
	}
	if body.Value.AutoBan != nil {
		if *body.Value.AutoBan {
			entry.AutoBan = 1
		} else {
			entry.AutoBan = 0
		}
	}
	if body.Value.ParamOverride != nil {
		entry.ParamOverride = strings.TrimSpace(*body.Value.ParamOverride)
	}
	if body.Value.HeaderOverride != nil {
		entry.HeaderOverride = strings.TrimSpace(*body.Value.HeaderOverride)
	}
	if body.Value.StatusCodeMapping != nil {
		entry.StatusCodeMapping = strings.TrimSpace(*body.Value.StatusCodeMapping)
	}
	if body.Value.AllowServiceTier != nil {
		entry.AllowServiceTier = *body.Value.AllowServiceTier
	}
	if body.Value.DisableStore != nil {
		entry.DisableStore = *body.Value.DisableStore
	}
	if body.Value.AllowSafetyIdentifier != nil {
		entry.AllowSafetyIdentifier = *body.Value.AllowSafetyIdentifier
	}
	if body.Value.AllowIncludeObfuscation != nil {
		entry.AllowIncludeObfuscation = *body.Value.AllowIncludeObfuscation
	}
	if body.Value.UpstreamModelUpdateAutoSyncEnabled != nil {
		entry.UpstreamModelUpdateAutoSyncEnabled = *body.Value.UpstreamModelUpdateAutoSyncEnabled
	}
...
func (h *Handler) PutClaudeKeys(c *gin.Context) {
	...
	preserveUnnamedEntryNames(arr, h.cfg.ClaudeKey, func(entry *config.ClaudeKey) *string {
		return &entry.Name
	}, claudeKeyIdentity)
	for i := range arr {
...
// nvidia-api-key: []OpenAICompatibility (dedicated view over openai-compatibility entries)
func (h *Handler) GetNVIDIACompat(c *gin.Context) {
	nvidia, _ := splitOpenAICompatByNVIDIA(h.cfg.OpenAICompatibility)
	c.JSON(200, gin.H{"nvidia-api-key": normalizedOpenAICompatibilityEntries(nvidia)})
}

func (h *Handler) PutNVIDIACompat(c *gin.Context) {
	data, err := c.GetRawData()
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	...
	for i := range arr {
		...
		arr[i].Name = forceNVIDIAProviderName(arr[i].Name)
		filtered = append(filtered, arr[i])
	}

	_, others := splitOpenAICompatByNVIDIA(h.cfg.OpenAICompatibility)
	h.cfg.OpenAICompatibility = append(others, filtered...)
	h.cfg.SanitizeOpenAICompatibility()
	h.persist(c)
}

func (h *Handler) PatchNVIDIACompat(c *gin.Context) {
	type nvidiaCompatPatch struct {
		Name                               *string                             `json:"name"`
		Priority                           *int                                `json:"priority"`
		Prefix                             *string                             `json:"prefix"`
		BaseURL                            *string                             `json:"base-url"`
	...
	var body struct {
		Name  *string            `json:"name"`
		Index *int               `json:"index"`
		Value *nvidiaCompatPatch `json:"value"`
	}
	if err := c.ShouldBindJSON(&body); err != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	...
	nvidiaEntries, others := splitOpenAICompatByNVIDIA(h.cfg.OpenAICompatibility)
	if len(nvidiaEntries) == 0 {
		c.JSON(404, gin.H{"error": "item not found"})
		return
	}

	targetIndex := -1
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(nvidiaEntries) {
		targetIndex = *body.Index
	}
	if targetIndex == -1 && body.Name != nil {
		match := strings.TrimSpace(*body.Name)
		for i := range nvidiaEntries {
			if strings.EqualFold(strings.TrimSpace(nvidiaEntries[i].Name), match) {
				targetIndex = i
				break
			}
		}
	...
	entry := nvidiaEntries[targetIndex]
	if body.Value.Name != nil {
		entry.Name = strings.TrimSpace(*body.Value.Name)
	}
	if body.Value.Priority != nil {
	...
	if body.Value.BaseURL != nil {
		trimmed := strings.TrimSpace(*body.Value.BaseURL)
		if trimmed == "" {
			nvidiaEntries = append(nvidiaEntries[:targetIndex], nvidiaEntries[targetIndex+1:]...)
			h.cfg.OpenAICompatibility = append(others, nvidiaEntries...)
			h.cfg.SanitizeOpenAICompatibility()
			h.persist(c)
			return
		}
	...
	normalizeOpenAICompatibilityEntry(&entry)
	entry.Name = forceNVIDIAProviderName(entry.Name)
	nvidiaEntries[targetIndex] = entry
...
func (h *Handler) DeleteNVIDIACompat(c *gin.Context) {
	nvidiaEntries, others := splitOpenAICompatByNVIDIA(h.cfg.OpenAICompatibility)
	if len(nvidiaEntries) == 0 {
		c.JSON(404, gin.H{"error": "item not found"})
		return
	...
	if all := strings.TrimSpace(c.Query("all")); strings.EqualFold(all, "true") || all == "1" {
		h.cfg.OpenAICompatibility = others
		h.cfg.SanitizeOpenAICompatibility()
		h.persist(c)
		return
	}
	...
	if name := strings.TrimSpace(c.Query("name")); name != "" {
		out := make([]config.OpenAICompatibility, 0, len(nvidiaEntries))
		for _, v := range nvidiaEntries {
			if !strings.EqualFold(strings.TrimSpace(v.Name), name) {
				out = append(out, v)
			}
		}
		h.cfg.OpenAICompatibility = append(others, out...)
		h.cfg.SanitizeOpenAICompatibility()
		h.persist(c)
		return
	}
	...
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, err := fmt.Sscanf(idxStr, "%d", &idx)
		if err == nil && idx >= 0 && idx < len(nvidiaEntries) {
			nvidiaEntries = append(nvidiaEntries[:idx], nvidiaEntries[idx+1:]...)
			h.cfg.OpenAICompatibility = append(others, nvidiaEntries...)
			h.cfg.SanitizeOpenAICompatibility()
			h.persist(c)
			return
...
func (h *Handler) PutVertexCompatKeys(c *gin.Context) {
	...
	preserveUnnamedEntryNames(arr, h.cfg.VertexCompatAPIKey, func(entry *config.VertexCompatKey) *string {
		return &entry.Name
	}, vertexCompatKeyIdentity)
	for i := range arr {
...
func (h *Handler) PutCodexKeys(c *gin.Context) {
	...
	preserveUnnamedEntryNames(arr, h.cfg.CodexKey, func(entry *config.CodexKey) *string {
		return &entry.Name
	}, codexKeyIdentity)
...
func (h *Handler) PatchCodexKey(c *gin.Context) {
	type codexKeyPatch struct {
		...
		BaseURL                            *string              `json:"base-url"`
		ProxyURL                           *string              `json:"proxy-url"`
		SupportedProtocols                 *string              `json:"supported-protocols"`
		ResponsesNonStreamTransport        *string              `json:"responses-nonstream-transport"`
	...
	if body.Value.SupportedProtocols != nil {
		entry.SupportedProtocols = strings.TrimSpace(*body.Value.SupportedProtocols)
	}
	if body.Value.ResponsesNonStreamTransport != nil {
		entry.ResponsesNonStreamTransport = strings.TrimSpace(*body.Value.ResponsesNonStreamTransport)
	}
	if body.Value.Models != nil {
...
func isNVIDIACompatName(name string) bool {
	normalized := strings.ToLower(strings.TrimSpace(name))
	if normalized == "" {
		return false
	}
	if normalized == "nvidia" || normalized == "nvidia-nim" || normalized == "nim" {
		return true
	}
	return strings.Contains(normalized, "nvidia")
}

func forceNVIDIAProviderName(name string) string {
	if isNVIDIACompatName(name) {
		return strings.TrimSpace(name)
	}
	return "nvidia"
}

func splitOpenAICompatByNVIDIA(entries []config.OpenAICompatibility) ([]config.OpenAICompatibility, []config.OpenAICompatibility) {
	if len(entries) == 0 {
		return nil, nil
	}
	nvidia := make([]config.OpenAICompatibility, 0)
	others := make([]config.OpenAICompatibility, 0, len(entries))
	for i := range entries {
		entry := entries[i]
		if isNVIDIACompatName(entry.Name) {
			nvidia = append(nvidia, entry)
			continue
		}
...
func normalizeCodexKey(entry *config.CodexKey) {
	...
	entry.BaseURL = strings.TrimSpace(entry.BaseURL)
	entry.ProxyURL = strings.TrimSpace(entry.ProxyURL)
	entry.SupportedProtocols = strings.TrimSpace(entry.SupportedProtocols)
	entry.ResponsesNonStreamTransport = strings.TrimSpace(entry.ResponsesNonStreamTransport)
	entry.Headers = config.NormalizeHeaders(entry.Headers)
	entry.ExcludedModels = config.NormalizeExcludedModels(entry.ExcludedModels)
	if len(entry.Models) == 0 {
		return
