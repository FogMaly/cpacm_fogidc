...
func (h *Handler) saveTokenRecord(ctx context.Context, record *coreauth.Auth) (string, error) {
	...
	// Keep in-memory auth state in sync with the newly written auth file.
	// Without this, management-side actions (e.g. quota fetch via auth_index)
	// can keep using stale tokens until process restart.
	h.syncSavedTokenRecord(ctx, record, savedPath)
	return savedPath, nil
}

func (h *Handler) syncSavedTokenRecord(ctx context.Context, record *coreauth.Auth, savedPath string) {
	...
	if errRead != nil {
		log.WithError(errRead).Warnf("failed to read saved auth file for manager sync: %s", savedPath)
		return
	}
	...
	if len(raw) > 0 {
		if errUnmarshal := json.Unmarshal(raw, &metadata); errUnmarshal != nil {
			log.WithError(errUnmarshal).Warnf("failed to parse saved auth metadata for manager sync: %s", savedPath)
			return
		}
	}
	...
	if id == "" {
		id = strings.TrimSpace(record.FileName)
	}
	if id == "" && h.cfg != nil {
		if rel, errRel := filepath.Rel(strings.TrimSpace(h.cfg.AuthDir), savedPath); errRel == nil && rel != "" && !strings.HasPrefix(rel, "..") && !filepath.IsAbs(rel) {
			id = rel
		}
	}
	if id == "" {
		id = filepath.Base(savedPath)
	}
	...
	if provider == "" {
		provider = authMetadataString(metadata, "type")
	}
	if provider == "" {
		provider = "unknown"
	}
	...
	attrs["path"] = savedPath
	if email := authMetadataString(metadata, "email"); email != "" {
		attrs["email"] = email
	}

	label := strings.TrimSpace(record.Label)
	if label == "" {
		label = authMetadataString(metadata, "email")
	}
	if label == "" {
		label = authMetadataString(metadata, "project_id")
	}

	modTime := time.Now().UTC()
	if info, errStat := os.Stat(savedPath); errStat == nil {
		modTime = info.ModTime().UTC()
	}

	existing, found := h.authManager.GetByID(id)
	var target *coreauth.Auth
	if found && existing != nil {
		target = existing.Clone()
	} else {
		target = &coreauth.Auth{ID: id}
	}
	...
	target.UpdatedAt = modTime
	if target.CreatedAt.IsZero() {
		target.CreatedAt = modTime
	}
	if !disabled && strings.EqualFold(strings.TrimSpace(target.StatusMessage), "disabled via management api") {
		target.StatusMessage = ""
	}

	var errSync error
	if found && existing != nil {
		_, errSync = h.authManager.Update(ctx, target)
	} else {
		_, errSync = h.authManager.Register(ctx, target)
	}
	if errSync != nil {
		log.WithError(errSync).Warnf("failed to sync auth manager record for %s", id)
	}
}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	go func() {
		...
		if h != nil && h.cfg != nil {
			existingPath := filepath.Join(h.cfg.AuthDir, antigravity.CredentialFileName(email))
			if raw, errRead := os.ReadFile(existingPath); errRead == nil && len(raw) > 0 {
				_ = json.Unmarshal(raw, &previousMetadata)
			}
		}

		if projectID == "" {
			if fallbackProjectID := authMetadataString(previousMetadata, "project_id"); fallbackProjectID != "" {
				projectID = fallbackProjectID
				log.Infof("antigravity: reused existing project ID %s", projectID)
			}
		}

		baseURL := authMetadataString(previousMetadata, "base_url")
		if baseURL == "" {
		...
		if refreshToken == "" {
			refreshToken = authMetadataString(previousMetadata, "refresh_token")
		}
