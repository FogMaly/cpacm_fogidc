...
func defaultRequestLoggerFactory(cfg *config.Config, configPath string) logging.RequestLogger {
	...
	if base := util.WritablePath(); base != "" {
		logger := logging.NewFileRequestLogger(cfg.RequestLog, filepath.Join(base, "logs"), configDir, cfg.ErrorLogsMaxFiles)
		logger.SetStreamBodyLimit(cfg.RequestLogStreamMaxBytes)
		return logger
	}
	logger := logging.NewFileRequestLogger(cfg.RequestLog, "logs", configDir, cfg.ErrorLogsMaxFiles)
	logger.SetStreamBodyLimit(cfg.RequestLogStreamMaxBytes)
	return logger
}
...
type Server struct {
	...
	// cpamsResolver is the CPAMS channel routing resolver.
	cpamsResolver *cpamsResolver

	// inboundLimiter is the optional inbound rate limiter.
	inboundLimiter *inboundLimiter

	covsActivation *covsactivation.Service
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if requestLogger != nil {
		engine.Use(middleware.RequestLoggingMiddleware(requestLogger, &cfg.SDKConfig))
	...
	s.covsActivation = covsactivation.NewService(cfg, configFilePath)
	covsactivation.RegisterAccessProvider(s.covsActivation)
	...
	// Initialize inbound limiter and CPAMS resolver
	s.inboundLimiter = newInboundLimiter(cfg)
	s.cpamsResolver = newCPAMSResolver(configFilePath)
	logCPAMSCodexConfigWarnings(cfg)
	if authManager != nil {
		snapshotPath := filepath.Join(managementasset.StaticDir(configFilePath), "model-health-multi.json")
		authManager.SetUnifiedProbeSnapshotPath(snapshotPath)
		authManager.SetUnifiedBalanceSource(func(ctx context.Context) []health.ProviderBalanceSnapshotItem {
			if s == nil || s.mgmt == nil {
				return nil
			}
			return s.mgmt.ProviderBalanceSnapshot(ctx)
		})
	}
	if s.cpamsResolver != nil {
		s.cpamsResolver.SetBalanceSource(func(ctx context.Context) []health.ProviderBalanceSnapshotItem {
			if s == nil || s.mgmt == nil {
				return nil
			}
			return s.mgmt.ProviderBalanceSnapshot(ctx)
		...
		if authManager != nil {
			s.cpamsResolver.SetRuntimeOutageSource(func(context.Context) []health.RuntimeOutageSnapshotItem {
				return authManager.RuntimeOutageSnapshot()
			})
		}
	}
	...
	s.ampModule = ampmodule.New(
		...
		ampmodule.WithUserRouteInterceptor(s.handleCOVSActivationAmpBypass),
	)
	ctx := modules.Context{
		Engine:         engine,
...
func (s *Server) setupRoutes() {
	...
	s.engine.GET("/model-health.json", s.serveManagementSnapshot("model-health.json"))
	s.engine.GET("/model-health-multi.json", s.serveManagementSnapshot("model-health-multi.json"))
	s.engine.GET("/provider-health-history.json", s.serveManagementSnapshot("provider-health-history.json"))
	...
	v1.Use(s.covsActivationRequestMiddleware())
	v1.Use(s.mgmt.LiveMonitorMiddleware())
	{
		v1.GET("/models", s.unifiedModelsHandler(openaiHandlers, claudeCodeHandlers))
		v1.POST("/chat/completions", s.unifiedChatCompletionsHandler(openaiHandlers.ChatCompletions, claudeCodeHandlers.ClaudeMessages))
		v1.POST("/completions", s.wrapCPAMSModelAliasRoute(openaiHandlers.Completions))
		v1.POST("/messages", s.wrapCPAMSModelAliasRoute(claudeCodeHandlers.ClaudeMessages))
		v1.POST("/messages/count_tokens", s.wrapCPAMSModelAliasRoute(claudeCodeHandlers.ClaudeCountTokens))
		v1.POST("/responses", s.wrapCPAMSModelAliasRoute(openaiResponsesHandlers.Responses))
		v1.POST("/responses/compact", s.wrapCPAMSModelAliasRoute(openaiResponsesHandlers.Compact))
	}

	// Gemini compatible API routes
	v1beta := s.engine.Group("/v1beta")
	v1beta.Use(AuthMiddleware(s.accessManager))
	v1beta.Use(s.mgmt.LiveMonitorMiddleware())
	{
	...
	s.registerCOVSActivationRoutes()

	// CPAMS multi-channel routing routes.
	s.registerCPAMSRoutes(openaiHandlers, claudeCodeHandlers, openaiResponsesHandlers)
}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/provider-balances", s.mgmt.GetProviderBalances)
		mgmt.GET("/provider-health", s.mgmt.GetProviderHealth)
		mgmt.GET("/load-balancer-runtime", s.mgmt.GetLoadBalancerRuntime)
		mgmt.GET("/live-monitor/cards", s.mgmt.GetLiveMonitorCards)
		mgmt.GET("/live-monitor/cards/:card_id", s.mgmt.GetLiveMonitorCard)
		mgmt.GET("/live-monitor/cards/:card_id/history", s.mgmt.GetLiveMonitorCardHistory)
		mgmt.GET("/live-monitor/stream", s.mgmt.StreamLiveMonitor)
		mgmt.GET("/runtime-status", s.getRuntimeStatus)
		mgmt.GET("/metrics", s.getMetrics)
		...
		mgmt.GET("/probe-scheduler", s.mgmt.GetProbeScheduler)
		mgmt.PUT("/probe-scheduler", s.mgmt.PutProbeScheduler)
		mgmt.PATCH("/probe-scheduler", s.mgmt.PutProbeScheduler)
...
func (s *Server) serveManagementControlPanel(c *gin.Context) {
	...
	data, err := injectManagementEnhancements(filePath)
	if err != nil {
		c.File(filePath)
		return
	}
...
func (s *Server) serveManagementSnapshot(name string) gin.HandlerFunc {
	return func(c *gin.Context) {
		if s == nil || s.mgmt == nil {
			c.AbortWithStatus(http.StatusNotFound)
			return
		}
		if !s.mgmt.AuthorizeRequest(c) {
			return
		}
		...
		if _, err := os.Stat(filePath); err != nil {
			...
			log.WithError(err).Errorf("failed to stat management snapshot asset %q", name)
			c.AbortWithStatus(http.StatusInternalServerError)
			return
		}
...
func (s *Server) unifiedChatCompletionsHandler(openaiHandler gin.HandlerFunc, claudeHandler gin.HandlerFunc) gin.HandlerFunc {
	openAICompat := s.wrapCPAMSModelAliasRoute(openaiHandler)
	claudeCompat := s.wrapCPAMSModelAliasRoute(claudeHandler)
	...
	return func(c *gin.Context) {
		...
		if cpamsLooksLikeNativeClaudeIngress(c, raw) {
			c.Set("cpams_allow_native_claude_alias_on_openai_route", true)
			c.Set("cpams_force_channel", "claude")
			claudeCompat(c)
			return
		}
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if s.covsActivation != nil {
		s.covsActivation.SetConfig(cfg)
	}
	...
	if oldCfg == nil ||
		...
		oldCfg.UsageDetailsMemoryWindowMinutes != cfg.UsageDetailsMemoryWindowMinutes ||
		oldCfg.UsageDetailsRetentionDays != cfg.UsageDetailsRetentionDays {
		usage.ConfigureRuntime(usage.RuntimeConfig{
			Enabled:             cfg.UsageStatisticsEnabled,
			DetailsStateDir:     filepath.Join(usage.ResolveStateDir(s.configFilePath, s.currentPath), "usage-details"),
			DetailsMemoryWindow: time.Duration(cfg.UsageDetailsMemoryWindowMinutes) * time.Minute,
			DetailsRetention:    time.Duration(cfg.UsageDetailsRetentionDays) * 24 * time.Hour,
		})
	}
	...
	if s.requestLogger != nil && (oldCfg == nil || oldCfg.RequestLogStreamMaxBytes != cfg.RequestLogStreamMaxBytes) {
		if setter, ok := s.requestLogger.(interface{ SetStreamBodyLimit(int) }); ok {
			setter.SetStreamBodyLimit(cfg.RequestLogStreamMaxBytes)
		}
	}
...
// it allows all requests (legacy behaviour). An optional inboundLimiter may be
// passed; when non-nil it enforces per-key QPS and global concurrency limits.
func AuthMiddleware(manager *sdkaccess.Manager, limiters ...*inboundLimiter) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if err == nil {
			if result != nil {
				...
				if limiter != nil && !limiter.AllowKey(result.Principal) {
