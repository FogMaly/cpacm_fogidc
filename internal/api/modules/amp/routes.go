...
func (m *AmpModule) managementAvailabilityMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		if m.getProxy() == nil {
			if m.userRouteInterceptor != nil && strings.HasPrefix(c.Request.URL.Path, "/api/user") {
				c.Next()
				return
			}
...
func (m *AmpModule) registerManagementRoutes(engine *gin.Engine, baseHandler *handlers.BaseAPIHandler, auth gin.HandlerFunc) {
	...
	if auth != nil {
		authWithBypass = wrapManagementAuth(
			auth,
			"/api/user/activate",
			"/threads",
			"/auth",
	...
	userProxyHandler := func(c *gin.Context) {
		if m.userRouteInterceptor != nil && m.userRouteInterceptor(c) {
			return
		}
		proxyHandler(c)
	}
	...
	ampAPI.Any("/user", userProxyHandler)
	ampAPI.Any("/user/*path", userProxyHandler)
