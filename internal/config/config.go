...
type Config struct {
	...
	// InboundRateLimit configures request admission controls at the proxy ingress.
	InboundRateLimit InboundRateLimitConfig `yaml:"inbound-rate-limit" json:"inbound-rate-limit"`
	...
	// COVSActivation configures the optional built-in card activation control plane.
	COVSActivation COVSActivationConfig `yaml:"covs-activation,omitempty" json:"covs-activation,omitempty"`

	legacyMigrationPending bool `yaml:"-" json:"-"`
}

// COVSActivationConfig configures the built-in activation service used to issue
// end-user API tokens backed by the proxy's configured upstream providers.
type COVSActivationConfig struct {
	...
	DefaultClaudeProviderPrefix string               `yaml:"default-claude-provider-prefix,omitempty" json:"default-claude-provider-prefix,omitempty"`
	Cards                       []COVSActivationCard `yaml:"cards,omitempty" json:"cards,omitempty"`
}

// COVSActivationCard seeds an activation card into the local state store.
type COVSActivationCard struct {
	Code         string `yaml:"code" json:"code"`
	Type         string `yaml:"type,omitempty" json:"type,omitempty"`
	Product      string `yaml:"product,omitempty" json:"product,omitempty"`
	DurationDays int    `yaml:"duration-days,omitempty" json:"duration-days,omitempty"`
...
type RemoteManagement struct {
	...
	// TrustedIPs bypass remote failure bans while still requiring a valid management key.
	TrustedIPs []string `yaml:"trusted-ips,omitempty"`
...
type RoutingConfig struct {
	// Engine selects the routing engine implementation.
	// Supported values: "v1" (default), "v2", "shadow".
	Engine string `yaml:"engine,omitempty" json:"engine,omitempty"`
	// Strategy selects the credential selection strategy.
	// Supported values: "round-robin" (default), "fill-first", "health-aware".
	Strategy string `yaml:"strategy,omitempty" json:"strategy,omitempty"`
	// ProviderConcurrency limits concurrent upstream requests per provider.
	ProviderConcurrency map[string]int `yaml:"provider-concurrency,omitempty" json:"provider-concurrency,omitempty"`
	// Scheduler configures runtime load-aware routing and sticky migration behavior.
	Scheduler RoutingSchedulerConfig `yaml:"scheduler,omitempty" json:"scheduler,omitempty"`
}

type RoutingSchedulerConfig struct {
	SoftOverloadThreshold float64                    `yaml:"soft-overload-threshold,omitempty" json:"soft-overload-threshold,omitempty"`
	HardOverloadThreshold float64                    `yaml:"hard-overload-threshold,omitempty" json:"hard-overload-threshold,omitempty"`
	RecoverThreshold      float64                    `yaml:"recover-threshold,omitempty" json:"recover-threshold,omitempty"`
	StickyBonus           float64                    `yaml:"sticky-bonus,omitempty" json:"sticky-bonus,omitempty"`
	StickyTTLSeconds      int                        `yaml:"sticky-ttl-seconds,omitempty" json:"sticky-ttl-seconds,omitempty"`
	CooldownSeconds       int                        `yaml:"cooldown-seconds,omitempty" json:"cooldown-seconds,omitempty"`
	HalfOpenProbeRatio    float64                    `yaml:"half-open-probe-ratio,omitempty" json:"half-open-probe-ratio,omitempty"`
	Metrics               RoutingMetricsConfig       `yaml:"metrics,omitempty" json:"metrics,omitempty"`
	Weights               RoutingLoadWeights         `yaml:"weights,omitempty" json:"weights,omitempty"`
	AuthScoreWeights      RoutingAuthScoreWeights    `yaml:"auth-score-weights,omitempty" json:"auth-score-weights,omitempty"`
	Providers             map[string]RoutingProvider `yaml:"providers,omitempty" json:"providers,omitempty"`
}

type RoutingMetricsConfig struct {
	RPSWindowSeconds     int `yaml:"rps-window-seconds,omitempty" json:"rps-window-seconds,omitempty"`
	ErrorWindowSeconds   int `yaml:"error-window-seconds,omitempty" json:"error-window-seconds,omitempty"`
	LatencyWindowSeconds int `yaml:"latency-window-seconds,omitempty" json:"latency-window-seconds,omitempty"`
}

type RoutingLoadWeights struct {
	Inflight       float64 `yaml:"inflight,omitempty" json:"inflight,omitempty"`
	StickySessions float64 `yaml:"sticky-sessions,omitempty" json:"sticky-sessions,omitempty"`
	TokenSlots     float64 `yaml:"token-slots,omitempty" json:"token-slots,omitempty"`
}

type RoutingAuthScoreWeights struct {
	Health  float64 `yaml:"health,omitempty" json:"health,omitempty"`
	Balance float64 `yaml:"balance,omitempty" json:"balance,omitempty"`
	Latency float64 `yaml:"latency,omitempty" json:"latency,omitempty"`
	Idle    float64 `yaml:"idle,omitempty" json:"idle,omitempty"`
}

type RoutingProvider struct {
	CapacityWeight float64 `yaml:"capacity-weight,omitempty" json:"capacity-weight,omitempty"`
	MaxCapacity    int     `yaml:"max-capacity,omitempty" json:"max-capacity,omitempty"`
}

// InboundRateLimitConfig defines simple ingress protection controls.
type InboundRateLimitConfig struct {
	// PerKeyQPS limits authenticated requests per second for each client API key.
	PerKeyQPS float64 `yaml:"per-key-qps,omitempty" json:"per-key-qps,omitempty"`
	// GlobalConcurrency limits the total number of in-flight admitted requests.
	GlobalConcurrency int `yaml:"global-concurrency,omitempty" json:"global-concurrency,omitempty"`
}
...
type ClaudeKey struct {
	...
	// NativePassthrough keeps Claude messages requests/responses as close to raw
	// upstream transport as possible while still allowing load balancing.
	NativePassthrough bool `yaml:"native-passthrough,omitempty" json:"native-passthrough,omitempty"`
...
type CodexKey struct {
	...
	// Name is an optional display name for management UI (e.g., "EE").
	Name string `yaml:"name,omitempty" json:"name,omitempty"`
	...
	// SupportedProtocols explicitly declares which request protocols this upstream supports.
	// Example: "responses, chat/completions".
	SupportedProtocols string `yaml:"supported-protocols,omitempty" json:"supported-protocols,omitempty"`

	// ResponsesNonStreamTransport controls how non-streaming OpenAI Responses requests
	// are sent upstream for this provider.
	// Supported values: "auto" (default), "json", "sse".
	ResponsesNonStreamTransport string `yaml:"responses-nonstream-transport,omitempty" json:"responses-nonstream-transport,omitempty"`
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	cfg.RequestLogMaxBodyBytes = 65536
	cfg.RequestLogStreamMaxBytes = 262144
	cfg.UsageDetailsMemoryWindowMinutes = 60
	cfg.UsageDetailsRetentionDays = 7
	cfg.Pprof.Enable = false
	cfg.Pprof.Addr = DefaultPprofAddr
	cfg.AmpCode.RestrictManagementToLocalhost = false // Default to false: API key auth is sufficient
	cfg.RemoteManagement.PanelGitHubRepository = DefaultPanelGitHubRepository
	cfg.Routing.Engine = "v1"
	...
	cfg.RemoteManagement.TrustedIPs = normalizeRemoteManagementTrustedIPs(cfg.RemoteManagement.TrustedIPs)
...
func (cfg *Config) SanitizeCodexKeys() {
	...
	for i := range cfg.CodexKey {
		...
		e.Prefix = normalizeModelPrefix(e.Prefix)
		e.BaseURL = strings.TrimSpace(e.BaseURL)
		e.SupportedProtocols = strings.TrimSpace(e.SupportedProtocols)
		e.ResponsesNonStreamTransport = strings.TrimSpace(e.ResponsesNonStreamTransport)
		e.Headers = NormalizeHeaders(e.Headers)
...
func isKnownDefaultValue(path []string, node *yaml.Node) bool {
	...
	if node.Kind == yaml.ScalarNode && node.Tag == "!!str" {
		...
		case "routing.engine":
			return node.Value == "v1"
		case "routing.strategy":
			return node.Value == "round-robin"
		case "request-log-stream-max-bytes":
			return node.Value == "262144"
		}
	}
	...
	if node.Kind == yaml.ScalarNode && node.Tag == "!!int" {
		...
		case "request-log-max-body-bytes":
			return node.Value == "65536"
		case "usage-details-memory-window-minutes":
			return node.Value == "60"
		case "usage-details-retention-days":
			return node.Value == "7"
		}
	}
...
func normalizeRemoteManagementTrustedIPs(values []string) []string {
	if len(values) == 0 {
		return nil
	}
