...
const (
	defaultStreamingLogBodyLimit  = 262144
	streamingLogChannelBufferSize = 16
)

var (
	toolFunctionNamePattern = regexp.MustCompile(`"function"\s*:\s*\{[^{}]*"name"\s*:\s*"([^"]+)"`)
	toolUseNamePattern      = regexp.MustCompile(`"type"\s*:\s*"tool_use"[^{}]*"name"\s*:\s*"([^"]+)"`)
	toolCallsNamePattern    = regexp.MustCompile(`"tool_calls"\s*:\s*\[[\s\S]*?"name"\s*:\s*"([^"]+)"`)
	statusLinePattern       = regexp.MustCompile(`(?m)^Status:\s*(\d{3})\s*$`)
	errorLinePattern        = regexp.MustCompile(`(?m)^Error:\s*(.+?)\s*$`)
)
...
type FileRequestLogger struct {
	...
	// streamBodyLimit caps how many bytes of streaming response bodies are retained in request logs.
	streamBodyLimit int
}
...
// SetStreamBodyLimit updates the maximum number of streaming response bytes retained in request logs.
func (l *FileRequestLogger) SetStreamBodyLimit(limit int) {
	l.streamBodyLimit = normalizeStreamBodyLimit(limit)
}
...
func (l *FileRequestLogger) LogStreamingRequest(url, method string, headers map[string][]string, body []byte, requestID string) (StreamingLogWriter, error) {
	...
	writer := &FileStreamingLogWriter{
		...
		bodyLimit:        normalizeStreamBodyLimit(l.streamBodyLimit),
		chunkChan:        make(chan []byte, streamingLogChannelBufferSize),
...
func (l *FileRequestLogger) writeNonStreamingLog(
	...
	if errWrite := writeResponseSection(w, statusCode, true, responseHeaders, bytes.NewReader(response), decompressErr, true); errWrite != nil {
		return errWrite
	}
	return writeSummarySection(w, buildRequestLogSummary(requestHeaders, requestBody, apiRequest, apiResponse, apiResponseErrors, statusCode, responseHeaders, response, decompressErr))
}
...
type requestLogSummary struct {
	...
	doneSeen           bool
	replyTailComplete  bool
	finalStatus        int
	upstreamAttempts   int
	upstreamStatusText string
	transportError     string
	cpamsChannel       string
	cpamsDecision      string
	requestedModel     string
	selectedModel      string
}

func buildRequestLogSummary(
	requestHeaders map[string][]string,
	requestBody []byte,
	apiRequest []byte,
	apiResponse []byte,
	...
	streamingResponse := isStreamingResponse(responseHeaders, response)
	toolCallSeen := hasToolCall(apiResponse, response)
	messageStopSeen := hasAnyMarker([][]byte{apiResponse, response}, `"type":"message_stop"`, `"type": "message_stop"`, "event: message_stop")
	doneSeen := hasAnyMarker([][]byte{apiResponse, response}, "[DONE]")
	firstPayloadSent := len(response) > 0
	replyTailComplete := doneSeen || messageStopSeen
	if !replyTailComplete && !streamingResponse && statusCode >= 200 && statusCode < 300 && firstPayloadSent && decompressErr == nil {
		replyTailComplete = true
	}

	upstreamStatuses := extractUpstreamStatusChain(apiResponse, apiResponseErrors)
	transportError := extractTransportError(apiResponse, apiResponseErrors)
	if transportError == "" && decompressErr != nil {
		transportError = decompressErr.Error()
	}

	return requestLogSummary{
		requestReceived:    len(requestBody) > 0 || len(requestHeaders) > 0,
		clientUserAgent:    firstHeaderValue(requestHeaders, "User-Agent"),
		streamingResponse:  streamingResponse,
		firstPayloadSent:   firstPayloadSent,
		toolCallSeen:       toolCallSeen,
		firstToolName:      extractFirstToolName(apiResponse, response),
		messageStopSeen:    messageStopSeen,
		doneSeen:           doneSeen,
		replyTailComplete:  replyTailComplete,
		finalStatus:        statusCode,
		upstreamAttempts:   len(upstreamStatuses),
		upstreamStatusText: joinOrDefault(upstreamStatuses, " -> ", "none"),
		transportError:     valueOrDefault(transportError, "none"),
		cpamsChannel:       valueOrDefault(firstHeaderValue(responseHeaders, "X-Cpams-Channel"), "unknown"),
		cpamsDecision:      valueOrDefault(firstHeaderValue(responseHeaders, "X-Cpams-Decision"), "unknown"),
		requestedModel:     valueOrDefault(firstHeaderValue(responseHeaders, "X-Cpams-Requested-Model"), "unknown"),
		selectedModel:      valueOrDefault(firstHeaderValue(responseHeaders, "X-Cpams-Model"), "unknown"),
	}
}

func writeSummarySection(w io.Writer, summary requestLogSummary) error {
	lines := []string{
		"=== SUMMARY ===\n",
		fmt.Sprintf("request_received: %s\n", yesNo(summary.requestReceived)),
		fmt.Sprintf("client_user_agent: %s\n", valueOrDefault(summary.clientUserAgent, "unknown")),
		fmt.Sprintf("streaming_response: %s\n", yesNo(summary.streamingResponse)),
		fmt.Sprintf("first_payload_sent: %s\n", yesNo(summary.firstPayloadSent)),
		fmt.Sprintf("tool_call_seen: %s\n", yesNo(summary.toolCallSeen)),
		fmt.Sprintf("first_tool_name: %s\n", valueOrDefault(summary.firstToolName, "none")),
		fmt.Sprintf("message_stop_seen: %s\n", yesNo(summary.messageStopSeen)),
		fmt.Sprintf("done_seen: %s\n", yesNo(summary.doneSeen)),
		fmt.Sprintf("reply_tail_complete: %s\n", yesNo(summary.replyTailComplete)),
		fmt.Sprintf("upstream_attempts: %d\n", summary.upstreamAttempts),
		fmt.Sprintf("upstream_status_chain: %s\n", summary.upstreamStatusText),
		fmt.Sprintf("transport_error: %s\n", summary.transportError),
		fmt.Sprintf("final_status: %d\n", summary.finalStatus),
		fmt.Sprintf("cpams_channel: %s\n", summary.cpamsChannel),
		fmt.Sprintf("cpams_decision: %s\n", summary.cpamsDecision),
		fmt.Sprintf("cpams_requested_model: %s\n", summary.requestedModel),
		fmt.Sprintf("cpams_model: %s\n", summary.selectedModel),
	}
	for _, line := range lines {
		if _, errWrite := io.WriteString(w, line); errWrite != nil {
			return errWrite
		}
	}
	return nil
...
func joinOrDefault(values []string, separator string, fallback string) string {
	if len(values) == 0 {
		return fallback
	}
...
func isStreamingResponse(responseHeaders map[string][]string, response []byte) bool {
	contentType := strings.ToLower(firstHeaderValue(responseHeaders, "Content-Type"))
	if strings.Contains(contentType, "text/event-stream") {
		return true
	}
	return hasAnyMarker([][]byte{response}, "event:", "[DONE]")
}

func hasToolCall(parts ...[]byte) bool {
	return hasAnyMarker(parts, `"tool_calls"`, `"tool_use"`, `"type":"tool_use"`, `"type": "tool_use"`)
}

func hasAnyMarker(payloads [][]byte, markers ...string) bool {
...
func extractFirstToolName(parts ...[]byte) string {
	patterns := []*regexp.Regexp{
		toolFunctionNamePattern,
		toolUseNamePattern,
		toolCallsNamePattern,
	}
...
func extractUpstreamStatusChain(apiResponse []byte, apiResponseErrors []*interfaces.ErrorMessage) []string {
	var statuses []string
	for _, match := range statusLinePattern.FindAllSubmatch(apiResponse, -1) {
		if len(match) >= 2 {
			statuses = append(statuses, string(match[1]))
		}
	}
	for _, match := range errorLinePattern.FindAllSubmatch(apiResponse, -1) {
		if len(match) >= 2 {
			statuses = append(statuses, "transport_error")
		}
	}
	if len(statuses) > 0 {
		return statuses
	...
	for _, apiErr := range apiResponseErrors {
		...
		if apiErr.StatusCode > 0 {
			statuses = append(statuses, fmt.Sprintf("%d", apiErr.StatusCode))
			continue
		}
		if apiErr.Error != nil {
			statuses = append(statuses, "transport_error")
...
func extractTransportError(apiResponse []byte, apiResponseErrors []*interfaces.ErrorMessage) string {
	matches := errorLinePattern.FindSubmatch(apiResponse)
	if len(matches) >= 2 {
	...
	for _, apiErr := range apiResponseErrors {
		...
		return strings.TrimSpace(apiErr.Error.Error())
	}
	return ""
}
...
func (l *FileRequestLogger) formatLogContent(url, method string, headers map[string][]string, body, apiRequest, apiResponse, response []byte, status int, responseHeaders map[string][]string, apiResponseErrors []*interfaces.ErrorMessage) string {
	...
	content.WriteString("\n")
	_ = writeSummarySection(&content, buildRequestLogSummary(headers, body, apiRequest, apiResponse, apiResponseErrors, status, responseHeaders, response, nil))
...
type FileStreamingLogWriter struct {
	...
	// bodyLimit caps how many streaming response bytes are spooled into the log body.
	bodyLimit int

	// bodyBytes tracks the number of response body bytes already written.
	bodyBytes int

	// bodyTruncated records whether the truncation marker has already been written.
...
func (w *FileStreamingLogWriter) WriteChunkAsync(chunk []byte) {
	if w.chunkChan == nil || len(chunk) == 0 || w.bodyTruncated.Load() {
		return
	}
...
func (w *FileStreamingLogWriter) asyncWriter() {
	...
	for chunk := range w.chunkChan {
		...
		if errWrite := w.writeChunk(chunk); errWrite != nil {
			select {
...
func (w *FileStreamingLogWriter) writeChunk(chunk []byte) error {
	if w == nil || w.responseBodyFile == nil || len(chunk) == 0 {
		return nil
	}

	limit := normalizeStreamBodyLimit(w.bodyLimit)
	if w.bodyBytes >= limit {
		return w.writeStreamingTruncationNotice(limit)
	}

	remaining := limit - w.bodyBytes
	toWrite := chunk
	...
	if len(toWrite) > 0 {
		if _, errWrite := w.responseBodyFile.Write(toWrite); errWrite != nil {
			return errWrite
		}
		w.bodyBytes += len(toWrite)
	}
...
func (w *FileStreamingLogWriter) writeStreamingTruncationNotice(limit int) error {
	if w == nil || w.responseBodyFile == nil || w.bodyTruncated.Load() {
		return nil
	}

	var notice bytes.Buffer
	if w.bodyBytes > 0 {
		if _, errWrite := notice.WriteString("\n\n"); errWrite != nil {
			return errWrite
		}
	}
	if _, errWrite := notice.WriteString(streamingTruncationNotice(limit)); errWrite != nil {
		return errWrite
	}
	if _, errWrite := notice.WriteString("\n"); errWrite != nil {
		return errWrite
	}
	if _, errWrite := w.responseBodyFile.Write(notice.Bytes()); errWrite != nil {
		return errWrite
	}
	w.bodyTruncated.Store(true)
	return nil
}

func normalizeStreamBodyLimit(limit int) int {
	if limit <= 0 {
		return defaultStreamingLogBodyLimit
	}
	return limit
}

func streamingTruncationNotice(limit int) string {
	return fmt.Sprintf("[stream log truncated after %d bytes]", limit)
}
...
func (w *FileStreamingLogWriter) writeFinalLog(logFile *os.File) error {
	...
	responseBody, errRead := io.ReadAll(responseBodyFile)
	if errRead != nil {
		return errRead
	}
	if errWrite := writeResponseSection(logFile, w.responseStatus, w.statusWritten, w.responseHeaders, bytes.NewReader(responseBody), nil, true); errWrite != nil {
		return errWrite
	}
	return writeSummarySection(logFile, buildRequestLogSummary(w.requestHeaders, nil, w.apiRequest, w.apiResponse, nil, w.responseStatus, w.responseHeaders, responseBody, nil))
}
